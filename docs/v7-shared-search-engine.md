...
Registry-level note: do **not** turn this into a blanket "every conflict component must be a clique" rule. Vanilla tridents intentionally have the non-clique component `Loyalty — Riptide — Channeling` where Loyalty and Channeling do not conflict. That topology does not break Plex, because the three alternatives produce different `blocksBitset` / child exclusion masks and therefore are not squashed into one choice. The data-integrity suite now documents the known non-clique conflict components so new bridge-style conflict topology fails loudly unless it is intentional and reviewed.
