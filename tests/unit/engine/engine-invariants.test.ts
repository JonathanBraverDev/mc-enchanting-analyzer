...
describe('Engine Architectural Invariants', () => {
    ...
    it('Invariant: Mass Conservation (1.0 Total)', async () => {
        ...
        const stats = await engine.getStats({ item: 'sword', xp: 30, material: 'diamond', threshold: 0, maxIterations: 1_000_000 });
    ...
    it('Invariant: Static Pruning Floor (No leakage between checkpoints)', async () => {
        ...
        const coarseStats = await engine.getStats({ item: 'sword', xp: 30, material: 'diamond', threshold: 0.1, useCache: false });
    ...
    it('Invariant: Legacy Book Restriction (Pre-1.7.2)', async () => {
        ...
        const stats = await engine.getStats({ item: 'book', xp: 30, material: 'book', threshold: 0, maxIterations: 1_000_000 });
