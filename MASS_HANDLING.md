...
- [Folded and Detailed Accounting Views](#folded-and-detailed-accounting-views)
...
| `rounding` | Active fixed-point split residue plus projection materialization loss that is not assignable to an exact public branch. | Rounding uncertainty. |
...
## Folded and Detailed Accounting Views

Checkpoint snapshots expose two accounting layers:

- `snapshot.mass` and analyzer `accounting` are the folded compatibility view. These buckets are the stable values used by UI, CLI, public API callers, and conservation checks.
- `snapshot.mass.details` is a diagnostic drill-down grouped by stage, operation, and bucket. It explains how the folded view was produced, but callers should fold it back to the public buckets before comparing semantic results.

The detailed view currently uses two stages:
...
The stage bridge at snapshot time is:

```text
search.resolved == projection.results.source
search.pending  == projection.pending.source

projection.results.source == projection.results.projected
  + projection.results.clueIncompatible
  + projection.results.loss

projection.pending.source == projection.pending.projected
  + projection.pending.clueIncompatible
  + projection.pending.loss
```

The folded public view is:
...
The projection stage is built for the checkpoint snapshot. It must not mutate the coordinator's search state.
