...
export class ClueAnalysisService {
     ...
     */
    public static calculateClueMass(combos: Map<PackedCombo, bigint>, indexToEnchant: number[]): Map<number, bigint> {
        ...
        for (const [packed, prob] of combos.entries()) {
            ...
            // Minecraft selects a random survivor from the list to be the clue
            ...
            for (let i = 0; i < count; i++) {
                ...
                if (share > 0n) {
                    const current = clueMass.get(e) ?? 0n;
                    clueMass.set(e, current + share);
                }
            }
        }
    ...
    public static conditionOnClue(
        ...
        const clueMasses = this.calculateClueMass(rawCombos, indexToEnchant);
        ...
        for (const [packed, pCombo] of rawCombos.entries()) {
            ...
            if (share > 0n) {
                ...
                totalMass += pConditioned;
                conditionedCombos.set(packed, pConditioned);
                
                // Re-aggregate any/rank/count stats
                const count = enchants.length;
