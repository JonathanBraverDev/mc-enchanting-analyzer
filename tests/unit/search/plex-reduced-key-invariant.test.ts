...
describe('Plex reduced frontier-key invariant', () => {
    it('accepts representative vanilla registry shapes', () => {
        for (const spec of [
            ...
            assert.strictEqual(result.ok, true, `${spec.version} ${spec.item}/${spec.material} should satisfy the reduced-key invariant`);
            assert.deepStrictEqual(result.conflicts, []);
            assert.ok(result.checkedStateCount > 0);
        }
    });

    it('rejects a mutated registry whose conflict graph allows multiple payload histories per structural state', () => {
        const result = checkPlexReducedKeyInvariant({
        ...
        assert.notStrictEqual(result.conflicts[0]!.firstPayload, result.conflicts[0]!.nextPayload);
    });
});
