...
describe('KeyUtils.getStatsKey', () => {
    it('produces the same key for identical inputs', () => {
        const k1 = KeyUtils.getStatsKey(1, 2, 15, 3);
        const k2 = KeyUtils.getStatsKey(1, 2, 15, 3);
        assert.strictEqual(k1, k2);
    });

    it('produces different keys for different catId', () => {
        const k1 = KeyUtils.getStatsKey(0, 2, 15, 3);
        const k2 = KeyUtils.getStatsKey(1, 2, 15, 3);
        assert.notStrictEqual(k1, k2);
    });

    it('produces different keys for different matId', () => {
        const k1 = KeyUtils.getStatsKey(1, 0, 15, 3);
        const k2 = KeyUtils.getStatsKey(1, 1, 15, 3);
        assert.notStrictEqual(k1, k2);
    });

    it('produces different keys for different level', () => {
        const k1 = KeyUtils.getStatsKey(1, 2, 10, 3);
        const k2 = KeyUtils.getStatsKey(1, 2, 20, 3);
        assert.notStrictEqual(k1, k2);
    });

    it('produces different keys for different guaranteedId', () => {
        const k1 = KeyUtils.getStatsKey(1, 2, 15, 0);
        const k2 = KeyUtils.getStatsKey(1, 2, 15, 1);
        assert.notStrictEqual(k1, k2);
    });
    ...
    it('produces the same key regardless of limit (cross-tier cache hit)', () => {
        ...
        const kCoarse = KeyUtils.getStatsKey(1, 2, 15, 3);
        const kUltra = KeyUtils.getStatsKey(1, 2, 15, 3);
