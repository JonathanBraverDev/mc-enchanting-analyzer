...
- Canonical golden snapshots must be generated by exhaustive V7 search (`exhaustive: true`) with full result limits.
- If a case cannot currently complete exhaustively, exclude it from the golden suite and document it as an engine-throughput blocker instead of committing a budgeted fixture as golden data.
